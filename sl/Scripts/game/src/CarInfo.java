...
//		osd.createText( chas.C_drag, Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line ); line++;
		...
		if( car.iteratePartsInit() )
		{
			...
			while( part = car.iterateParts() )
			{
				...
				if ( part instanceof CylinderHead )
				{
					has_heads++;
					head = part;
					itkvalves = head.number_of_intake_valves_per_cylinder;
					exhvalves = head.number_of_exhaust_valves_per_cylinder;
					if (head instanceof OHV_CylinderHead )
						headType = "OHV";
					else
					if(head instanceof SOHC_CylinderHead)
						headType = "SOHC";
					else
					if(head instanceof DOHC_CylinderHead)
						headType = "DOHC";
				}
				else
				if(part instanceof TurboCharger)
					hasTurbo=1;
				else
				if(part instanceof SuperCharger)
					hasSupercharger=1;
			}
		}
		...
		*/
		...
		if( engine )
		{
			...
			if(dyno.nitro_consumption > 0)
				hasNitrous=1;
			
			
			if(engine instanceof Block_Inline) engType = "Inline";
			if(engine instanceof Block_Vee) engType = "V";
			if(engine instanceof Block_Vee && dyno.cylinders == 4) engType = "Flat";
			...
			if(its_vee)
			{
				if(has_heads > 1)
				{	
					osd.createText( engType + val + " " + headType + " " + nvalves + "valve", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
				}
				else
				{
					osd.createText( engType + val + " " + headType, Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
				}
			}
			else
			{
			...
			osd.createText( val +  " cc / " + Float.toString(dyno.Displacement*1000000.0*0.061, "%1.1f cui"), Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line); line++;
			osd.createText( "Aspiration:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line);
			if(hasNitrous == 0 && hasSupercharger == 0 && hasTurbo == 0)
			{
				osd.createText( "Naturally Aspirated", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			}
			else
			if(hasNitrous == 1 && hasSupercharger == 0 && hasTurbo == 0)
			{
				osd.createText( "Nitrous", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			}
			else
			if(hasNitrous == 0 && hasSupercharger == 1 && hasTurbo == 0)
			{
				osd.createText( "Supercharged", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			}
			else
			if(hasNitrous == 0 && hasSupercharger == 0 && hasTurbo == 1)
			{
				osd.createText( "Turbocharged", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			}
			else
			if(hasNitrous == 1 && hasSupercharger == 1 && hasTurbo == 0)
			{
				osd.createText( "Supercharged & Nitrous", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			}
			else
			if(hasNitrous == 1 && hasSupercharger == 0 && hasTurbo == 1)
			{
				osd.createText( "Turbocharged & Nitrous", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			}
			else
			if(hasNitrous == 0 && hasSupercharger == 1 && hasTurbo == 1)
			{
				osd.createText( "Twincharged", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			}
			else
			if(hasNitrous == 1 && hasSupercharger == 1 && hasTurbo == 1)
			{
				osd.createText( "ALL OF IT", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			}
			line++;
			if(dyno.P_turbo_waste > 0)
			{
				osd.createText( "Boost:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line);
				osd.createText( Float.toString(dyno.P_turbo_waste, "%1.2f bar") + " / " + Float.toString(dyno.P_turbo_waste*14.5038, "%1.1f psi"), Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
				line++;
			}
			if(dyno.nitro_consumption > 0)
			{
				osd.createText( "N2O:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line);
				osd.createText( Float.toString((dyno.nitro_consumption/0.333*132.0)/3.6, "%1.0f hp shot"), Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
				line++;
			}
			osd.createText( "Fuel type:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line);
			osd.createText( dyno.fuelType, Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line); line++;
			
			osd.createText( "Bore:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line);
			osd.createText( Float.toString(dyno.bore*1000.0, "%1.1f mm") + " / " + Float.toString(dyno.bore*1000.0/25.4, "%1.2f in"), Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line); line++;

			osd.createText( "Stroke:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line);
			if (has_crank)
				osd.createText( Float.toString(dyno.stroke*1000.0, "%1.1f mm") + " / " + Float.toString(dyno.stroke*1000.0/25.4, "%1.2f in"), Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line);
			else
			...
			osd.createText( "Gear Ratios:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line); line++;
			...
			osd.createText( "Final Drive:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line);
			osd.createText( chas.rearend_ratio+":1", Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line); line++;
			osd.createText( "Powertrain layout:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line );
			...
			osd.createText( "Engine Weight:", Frontend.smallFont, Text.ALIGN_LEFT, xpos, ypos, line);
			osd.createText( Float.toString(engine.getMass(), "%1.1f kg"), Frontend.smallFont, Text.ALIGN_LEFT, xpos2, ypos, line); line++;
		...
		if (error_text)
			...
			if (dyno)
			{
				...
				osd.createText( "Max Power = " + Float.toString((((dyno.getTorque(dyno.RPM_maxHP, 0.0) * 0.7376)*dyno.RPM_maxHP)/5252.0), "%1.2f HP / ") + Float.toString((((dyno.getTorque(dyno.RPM_maxHP, 0.0) * 0.7376)*dyno.RPM_maxHP)/5252.0*0.7457), "%1.2f Kw"), Frontend.smallFont, Text.ALIGN_LEFT, -0.07, -0.55, 0);
				osd.createText( "Max Torque = " + Float.toString(dyno.maxTorque*0.7353, "%1.2f lb-ft / ") + Float.toString(dyno.maxTorque, "%1.2f Nm"), Frontend.smallFont, Text.ALIGN_LEFT, -0.07, -0.50, 0);
